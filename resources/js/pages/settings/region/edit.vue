<script setup>
    ...
    const form = useForm({
        ...
        country: props.region.country_id,
        _prev: props.prev
    })

    function submit() {
        form.put(`/settings/region/${props.region.id}`, {
            preserveScroll: true
        })
    }
...
<template>
    ...
    <div>
        <Head>
            <title>{{ $t('settingsRegionEdit') }}</title>
        </Head>
        <div class="mx-auto max-w-screen-xl px-4 lg:px-12">
            <page-title 
                :title="$t('settingsRegion')"
                :subtitle="$t('settingsRegionEdit')"></page-title>

            <div class="max-w-[600px] p-6 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">                
                <h5 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">{{ $t('settingsRegionEdit') }}</h5>
                <hr class="mt-4 mb-4">
                ...
                <form @submit.prevent="submit">
                    ...
                    <div class="mb-3 font-normal text-gray-700 dark:text-gray-400">
                        <div class="mb-6">
                            <label for="name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">{{ $t('regionListName') }}</label>
                            <input 
                                type="text" 
                                id="name" 
                        ...
                        <div class="mb-6">
                            <label for="country" class="block mb-1 text-sm font-medium text-gray-900 dark:text-white">{{ $t('regionListCountryName') }}</label>
                            <select id="country" 
