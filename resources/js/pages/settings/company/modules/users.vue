<script setup>
   ...
   async function getUsersData() {
      try {
         ...
         let responseData = await axios({
            method: 'get',
            url: `/settings/company/${props.companyid}/users`
         }).then((response) => {
            return response;
         }).catch((error) => {
   ...
   async function storeOrUpdate() {
      try {
         if(frmType.value !== '') {
            ...
            let method = 'post';
            let url = `/settings/company/${props.companyid}/users`;
            if(frmType.value == 'edit') {
               method = 'put'
               url = `/settings/company/${props.companyid}/users/${form.id}`;
            }
   ...
   async function getUserData() {
      try {
         ...
         let responseData = await axios({
            method: 'get',
            url: `/settings/company/${props.companyid}/users/${form.id}/edit`
         }).then((response) => {
            return response;
         }).catch((error) => {
            return error.response
   ...
   async function confirmDelete() {
      try {
         let responseData = await axios({
               method: 'POST',
               url: `/settings/company/${props.companyid}/users/${_deleteId.value}`,
               data: {
                  _method: 'DELETE'
               }
            }).then((response) => {
   ...
   function _setSelected(id) {
      _deleteId.value = id;
      emit('confirm-open', 'users'); // if confirms only users component listen it
   }
   ...
   watch(() => bus.value.get('confirm-delete-users'), () => {
      confirmDelete()
   })
   ...
   onMounted(() => {
      initFlowbite();      
      frmModal.value = new Modal(document.getElementById('form-modal_users'), modalOptions) // Assigning Instanse 
      getUsersData();
   })
...
<template>
   <div>
      ...
      <form-modal 
         @close="closeModal"
         suffix="_users"
         :header="frmType == 'edit' ? $t('settingsUserEdit') : $t('settingsUserAdd')">
         ...
         <form class="space-y-3" @submit.prevent="storeOrUpdate">
            <div>
               ...
               <p class="mt-1 text-xs text-red-600 dark:text-red-500" v-if="formErrors.name">{{ formErrors.name[0] }}</p>
            </div>
            <div>
               ...
               <p class="mt-1 text-xs text-gray-500 dark:text-gray-400"> {{ $t('userEmailHelper') }} </p>
            ...
            <div>
               ...
               <input type="password" 
                      ...
                      placeholder="*****" />
               <p class="mt-1 text-xs text-gray-500 dark:text-gray-400"> {{ $t('userPasswordHelper') }} </p>
               <p class="mt-1 text-xs text-red-600 dark:text-red-500" v-if="formErrors.password">{{ formErrors.password[0] }}</p>                  
            </div>
            <button 
               type="submit"
               :disabled="frmLoader"
      ...
      <transition name="fade" mode="out-in" appear>
         ...
         <div v-else key="listdata" class="overflow-x-auto">
            ...
            <div class="flex items-center justify-between mb-4">
               <h5 class="text-xl font-bold leading-none text-gray-900 dark:text-white">{{ $t('companyLinkedUsers') }}</h5>
               <button
            ...
            <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400" v-if="usersdata.length > 0">
               <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                   <tr>
                       ...
                       <th scope="col" class="px-4 py-3">{{ $t('userEmail') }}</th>
