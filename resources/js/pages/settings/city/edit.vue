<script setup>
    ...
    const form = useForm({
        ...
        country: props.city.region.country_id,
        region: props.city.region_id,
        _prev: props.prev
    })

    function submit() {
        form.put(`/settings/city/${props.city.id}`, {
            preserveScroll: true
        })
    }
    ...
    async function onChangeCountry() {
      try {
         let responseData = await axios({
            method: 'get',
            url: `/settings/city/get-regions?country=${form.country}`
         }).then((response) => {
            return response;
         }).catch((error) => {
...
<template>
    ...
    <div>
        <Head>
            <title>{{ $t('settingsCityEdit') }}</title>
        </Head>
        <div class="mx-auto max-w-screen-xl px-4 lg:px-12">
            <page-title 
                :title="$t('settingsCity')"
                :subtitle="$t('settingsCityEdit')"></page-title>

            <div class="max-w-[600px] p-6 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">                
                <h5 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">{{ $t('settingsCityEdit') }}</h5>
                <hr class="mt-4 mb-4">
                ...
                <form @submit.prevent="submit">
                    ...
                    <div class="mb-3 font-normal text-gray-700 dark:text-gray-400">
                        <div class="mb-6">
                            <label for="name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">{{ $t('cityListName') }}</label>
                            <input 
                                type="text" 
                                id="name" 
                        ...
                        <div class="mb-6">
                            <label for="country" class="block mb-1 text-sm font-medium text-gray-900 dark:text-white">{{ $t('cityListCountryName') }}</label>
                            <select id="country"
                         ...
                         <div class="mb-6">
                            <label for="region" class="block mb-1 text-sm font-medium text-gray-900 dark:text-white">{{ $t('cityListRegionName') }}</label>
                            <select id="region" 
                                    v-model="form.region"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
