<script setup>
    ...
    function submit() {
        form.put(`/company/hr/jobtype/${props.jobtype.id}`, {
            preserveScroll: true
        })
    }
...
<template>
    ...
    <div>
        <Head>
            <title>{{ $t('hrJobTypeEdit') }}</title>
        </Head>
        <div class="mx-auto max-w-screen-xl px-4 lg:px-12">
            <page-title 
                :title="$t('hrJobType')"
                :subtitle="$t('hrJobTypeEdit')"></page-title>

            <div class="max-w-[600px] p-6 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">                
                <h5 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">{{ $t('hrJobTypeEdit') }}</h5>
                <hr class="mt-4 mb-4">
