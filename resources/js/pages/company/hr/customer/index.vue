...
function filter() {
    router.get('/company/hr/customer', { search: search.value }, { replace: true, preserveState: true })
}
...
function confirmDelete() {
    ...
    router.delete(`/company/hr/customer/${_deleteId.value}`, { 
        replace: true, 
        preserveState: true,
        onSuccess: () => {
...
<template>
    <Head :title="$t('hrCustomer')" />      
    <div class="mx-auto max-w-screen-xl px-4 lg:px-12">
        <page-title 
            :title="$t('hrCustomer')"
            :subtitle="$t('hrCustomerList')" />
        ...
        <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
            <div class="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
                ...
                <div class="w-full md:w-auto flex flex-col md:flex-row space-y-2 md:space-y-0 items-stretch md:items-center justify-end md:space-x-3 flex-shrink-0">
                    <Link href="/company/hr/customer/create" as="button" class="flex items-center justify-center text-white bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 font-medium rounded-lg text-sm px-4 py-2 dark:bg-primary-600 dark:hover:bg-primary-700 focus:outline-none dark:focus:ring-primary-800">
                ...
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400" v-if="listdata.data.length > 0">
                    <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                        <tr>
                            ...
                            <th scope="col" class="px-4 py-3">{{ $t('hrCustomerName') }}</th>
                            <th scope="col" class="px-4 py-3">{{ $t('hrCustomerContactPerson') }}</th>
                            <th scope="col" class="px-4 py-3">{{ $t('hrCustomerPhone') }}</th>
                            ...
                            <td class="px-4 py-3 flex items-center justify-end">
                                ...
                                <div 
                                    ...
                                    <ul class="py-1 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="apple-imac-27-dropdown-button">
                                        ...
                                        <li> <Link :href="`/company/hr/customer/${item.id}/edit`" class="block py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">{{ $t('edit') }} </Link> </li>
                                    </ul>
                                    <div class="py-1">
