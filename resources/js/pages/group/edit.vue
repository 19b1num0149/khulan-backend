<script setup>
    ...
    const form = useForm({
        ...
        founded_year: props.group.founded_year,
        admin: props.group.user,
        description: props.group.description,
        _prev: props.prev
    })

    function submit() {
        form.put(`/group/item/${props.group.id}`, {
            preserveScroll: true
        })
    }
</script>
...
<template>
    ...
    <div>
        ...
        <div class="mx-auto max-w-screen-xl px-4 lg:px-12">
            <page-title 
                :title="$t('groupInfo')"
                :subtitle="$t('groupEdit')"></page-title>

            <div class="max-w-[600px] p-6 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">                
                <h5 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">{{ $t('groupEdit') }}</h5>
                <hr class="mt-4 mb-4">
                ...
                <form @submit.prevent="submit">
                    ...
                    <div class="mb-3 font-normal text-gray-700 dark:text-gray-400">
                        <div class="mb-6">
                            <label for="name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">{{ $t('groupName') }}</label>
                            <input 
                                type="text" 
                                id="name" 
